...
static const struct rte_eth_conf mehcached_port_conf = {
	...
	.rx_adv_conf = {
		.rss_conf = {
			.rss_key = NULL,
			.rss_hf = ETH_RSS_IPV4 | ETH_RSS_UDP,
		},
	},
	.txmode = {
...
static struct rte_mempool *mehcached_pktmbuf_pool_nicmem[MEHCACHED_MAX_LCORES][MEHCACHED_MAX_PORTS];
...
mehcached_packet_alloc_nicmem(int port_id)
{
	...
	if (!mehcached_pktmbuf_pool_nicmem[lcore][port_id]) {
		rte_exit(EXIT_FAILURE, "error missing pool for port %d\n", port_id);
		return NULL;
	}
	return rte_pktmbuf_alloc(mehcached_pktmbuf_pool_nicmem[lcore][port_id]);
}
...
static int alloc_nicmem(uint8_t lcore, uint64_t port_id, uint16_t priv_size) {
	struct rte_mempool *nicmem_pktmbuf_pool = NULL;
	struct rte_pktmbuf_extmem ext_mem[2048];
	int ext_mem_num = 1;
	int totsz = 0, i = 0, ret;
	char pool_name[64];

	ext_mem[0].elt_size = RTE_MBUF_DEFAULT_BUF_SIZE;
	ext_mem[0].buf_len = MEHCACHED_MBUF_SIZE * ext_mem[0].elt_size;
	...
	while (totsz < MEHCACHED_MBUF_SIZE) {
		ext_mem[++i] = ext_mem[0];
	...
	const unsigned int cache_size = MEHCACHED_MAX_PORTS * 512;

	printf("initializing pktmbuf pool nicmem lcore %d\n", lcore);
	snprintf(pool_name, sizeof(pool_name), "nicmem_%zu_%zu", lcore, port_id);
	nicmem_pktmbuf_pool = rte_pktmbuf_pool_create_extbuf(pool_name, MEHCACHED_MBUF_SIZE,
						cache_size, priv_size,
						ext_mem[0].elt_size,
						0, &ext_mem[0],
						ext_mem_num);
	if (nicmem_pktmbuf_pool == NULL)
		rte_exit(EXIT_FAILURE, "Cannot init nicmem mbuf pool %d\n", lcore);
	mehcached_pktmbuf_pool_nicmem[lcore][port_id] = nicmem_pktmbuf_pool;
...
mehcached_init_network(uint64_t cpu_mask, uint64_t port_mask, uint8_t *out_num_ports, int nicmem, int inline_min, rte_post_tx_callback_fn txcb, uint16_t priv_size)
{
	...
	for (i = 0; i < num_numa_nodes; i++)
	{
		...
		mehcached_pktmbuf_pool[i] = rte_pktmbuf_pool_create(pool_name, MEHCACHED_MBUF_SIZE, cache_size, 0, MEHCACHED_MBUF_ENTRY_SIZE, i);
	...
	uint8_t port_id, lcore_id;
	memset(mehcached_pktmbuf_pool_nicmem, 0, sizeof(mehcached_pktmbuf_pool_nicmem));
	if (nicmem) {
		RTE_ETH_FOREACH_DEV(port_id)
		{
			...
			RTE_LCORE_FOREACH(lcore_id)
				alloc_nicmem(lcore_id, port_id, priv_size);
		}
	}
	...
	*out_num_ports = num_ports;
	...
	RTE_ETH_FOREACH_DEV(port_id)
	{
		...
		RTE_LCORE_FOREACH(lcore)
		{
			...
			memcached_tx_conf->inline_min = inline_min;
			...
			if (txcb) {
				ret = rte_eth_post_tx_callback_set(port_id, queue, txcb, NULL);
				if (ret) {
					fprintf(stderr, "failed to configure TX callback on port %hhu tx_queue %hu (err=%d)\n", port_id, queue, ret);
					return false;
				}
			}
		}
...
mehcached_set_dst_port_mapping(uint8_t port_id, uint16_t l4_dst_port, uint32_t lcore)
{
	...
	// uint16_t queue = (uint16_t)lcore;
	printf("Mapping dst port %d to queue %d\n", l4_dst_port, lcore);
